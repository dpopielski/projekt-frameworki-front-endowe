    ...
    return (
        <article className="bg-white rounded shadow">
            <div className="border-solid border-b-2 border-gray-200">
                <div className="p-3">
                    <img className="w-12 h-12 rounded-full mx-auto" src={PersonData.img} alt=""/>
            ...
            <div className="flex flex-col p-3 gap-2">
                {PersonalNavData.map((elem, index) => 
                    <div key={index} className="flex align-center justify-between">
                        <img className="w-4 h-auto my-auto mr-1.5" src={elem.icone1} alt=""/>
                        <span className="font-normal my-auto w-full text-xs mr-1.5">{elem.title}</span>
                        <img className="float-right w-8 h-5 p-1 rounded-md border-solid border border-gray-500" src={elem.icone2} alt=""/>
                    </div>
                )}
            </div>
        </article>
